...
import { FormacaoWithProgresso } from "../devolutivas/types";
import { Button } from "../ui/button";
import { Card, CardContent, CardFooter } from "../ui/card";

type FormacaoContentProps = {
  formacao: FormacaoWithProgresso;
  formacaoSlug: string;
};

export default function FormacaoContent({ formacao, formacaoSlug }: FormacaoContentProps) {
  ...
  const handleClick = (capacitacao: {
    ...
    const capacitacaoSlug = createSlug(capacitacao.nome);
    router.push(`/formacao/${formacaoSlug}/workshops/${capacitacaoSlug}`);
  };
            ...
            <CardFooter className="bg-gray-50 dark:bg-gray-800 p-4 border-t">
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
                <div 
                  ...
                  style={{ 
                    width: `${capacitacao.totalWorkshops === 0 ? 0 : (capacitacao.completedWorkshops / capacitacao.totalWorkshops) * 100}%` 
                  }}
                />
              </div>
            </CardFooter>
